...
function Loading() {
  ...
  return (
    <Html center style={{ fontFamily: 'system-ui', fontSize: 14, color: '#111' }}>
      Loading… {Math.round(progress)}%
    </Html>
  )
}

class SceneErrorBoundary extends Component<{ children: ReactNode }, { error: unknown | null }> {
  ...
  render() {
    if (this.state.error) {
      return (
        <Html center style={{ fontFamily: 'system-ui', fontSize: 12, color: '#b00020', maxWidth: 520 }}>
...
function Ring({ url, frame, diamonds, env, ...props }: any) {
  ...
  const diamondTargets = useMemo(() => {
    ...
    scene.traverse((obj: any) => {
      ...
      const materialName = obj.material?.name ? String(obj.material.name).toLowerCase() : ''
      const objectName = obj.name ? String(obj.name).toLowerCase() : ''
      const looksTransparent = Boolean(obj.material?.transparent) || (typeof obj.material?.opacity === 'number' && obj.material.opacity < 1)
      const looksTransmissive = typeof obj.material?.transmission === 'number' && obj.material.transmission > 0.05
...
function Scene({
  ...
  const hdr = useEnvironment({ files: 'https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/1k/studio_small_08_1k.hdr' })
  ...
  useEffect(() => {
    ...
    const offset = new THREE.Vector3(...CAMERA_ORBIT_OFFSET).normalize().multiplyScalar(distance)
    ...
    controlsRef.current.maxDistance = Math.max(controlsRef.current.minDistance + 0.1, distance * 1.25)
  ...
  return (
    <>
      <group ref={ringGroupRef}>
        <Center>
          <Ring key={modelUrl} url={modelUrl} frame={frame} diamonds={diamonds} env={hdr} scale={ringScale} rotation={INITIAL_RING_ROTATION} />
        </Center>
      </group>
      <OrbitControls
        ref={controlsRef}
      ...
      {postprocessing ? (
        <EffectComposer>
          {ppAO ? <N8AO aoRadius={aoRadius} intensity={aoIntensity} distanceFalloff={aoDistanceFalloff} /> : null}
          {ppBloom ? <Bloom luminanceThreshold={bloomThreshold} intensity={bloomIntensity} levels={6} mipmapBlur /> : null}
          {ppToneMapping ? <ToneMapping /> : null}
        </EffectComposer>
      ) : null}
      <Environment map={hdr} background={false} blur={0.7}>
        <Lightformer intensity={2.4} position={[0, 5, -6]} scale={[10, 10, 1]} />
        <Lightformer intensity={1.6} position={[0, -5, -6]} scale={[10, 10, 1]} />
        <Lightformer intensity={2.0} position={[6, 0, -3]} scale={[7, 7, 1]} />
        <Lightformer intensity={2.0} position={[-6, 0, -3]} scale={[7, 7, 1]} />
      </Environment>
      <Preload all />
    </>
...
export default function MarifetArPage() {
  ...
  const metalPreset = METAL_PRESETS.find((preset) => preset.key === metal) ?? METAL_PRESETS[0]
  const modelUrl = MODEL_URLS[model as keyof typeof MODEL_URLS] ?? MODEL_URLS.EM

  return (
    <div className={styles.app}>
      <div className={styles.modelPicker} role="group" aria-label="Model seçimi">
        {Object.keys(MODEL_URLS).map((key) => (
          <button key={key} className={model === key ? styles.activeButton : undefined} onClick={() => setModel(key)} type="button">
            {key}
          </button>
        ))}
      </div>

      <div className={styles.metalPicker} role="group" aria-label="Metal seçimi">
        {METAL_PRESETS.map((preset) => (
          <button
            key={preset.key}
            className={metal === preset.key ? styles.activeButton : undefined}
      ...
      >
        ...
        <spotLight color="#fff2de" position={[10, 10, 10]} angle={0.15} penumbra={1} decay={0} intensity={Math.PI} />
        <AdaptiveDpr />
        <AdaptiveEvents />
        <Stats showPanel={0} />
        <SceneErrorBoundary>
